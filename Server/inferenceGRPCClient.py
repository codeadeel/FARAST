...
class faceClient:
    def __init__(self, serverIp: string, idLen: int = 10, messageLen: int = 1000000000, clientName: string = None) -> None:
        ...
        self.stub = faceCommunication_pb2_grpc.faceServiceStub(self.channel)
    ...
    def __inputProcessor__(self, inp1: np.array, detectionConfidence: list, nmsThreshold: list, boxCushion: list, boxArea: list, recogThres: list, fakeThres: list, trackerFrames: list) -> faceCommunication_pb2.serverInput:
        """
        ...
        boxArea : Small face box area percentage
        recogThres : Face recognition threshold
        fakeThres : Face Anti-Spoofing threshold
        ...
        return faceCommunication_pb2.serverInput(
            imgs = inp1.tobytes(),
            batch = inp1Shape[0],
            width = inp1Shape[2],
            height = inp1Shape[1],
            ...
            boxArea = json.dumps(boxArea),
            recogThres = json.dumps(recogThres),
            fakeThres = json.dumps(fakeThres),
            trackerFrames = json.dumps(trackerFrames),
            trackerIds = json.dumps(self.streamPool),
            clientId = self.clientName
        )

    def __call__(self, imgList: list, detectionConfidence: list, nmsThreshold: list, boxCushion: list, boxArea: list, recogThres: list, fakeThres: list, trackerFrames: list) -> string:
        """
        ...
        """
        x = np.stack(imgList)
        resp = self.stub.inference(self.__inputProcessor__(x, detectionConfidence, nmsThreshold, boxCushion, boxArea, recogThres, fakeThres, trackerFrames))
...
if __name__=='__main__':
    parser = argparse.ArgumentParser(description = 'Face Detection, Recogntion, Anti-Spoofing & Tracking Inference Client.')
    ...
    faceC = faceClient(args['server_ip'])
    img = ocv.imread(args['link'])
    st = time.time()
    ret = faceC([img], [args['detectionThreshold']], [args['nmsThreshold']], [args['boxCushion']], [args['smallFacePercent']], [args['recognitionThreshold']], [args['spoofingThreshold']], [args['trackerFrames']])
    et = time.time() - st
    print(f'Inference Received!!! | Request Processing Time: {et}')
    print('-' * 30)
    print(ret)
